...
export default function MessagesPage() {
  ...
  async function loadConversations() {
    ...
    if (!res.ok || !res.data) {
      setTone("error");
      setStatus(res.error ?? "Could not load conversations.");
      return;
    }
  ...
  async function ensureConversationFromQuery() {
    ...
    if (!vendorId && !serviceId && !requestId) return;

    const res = await apiPost<{ ok: boolean; conversation: Conversation }>("/chat/conversations", {
      vendorId,
      serviceId,
      requestId,
      initialMessage,
  ...
  async function startCall() {
    ...
    setActiveCall(call);
    const otherUserId = conversation.vendorUserId;
